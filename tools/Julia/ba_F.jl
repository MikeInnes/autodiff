...
function rodrigues_rotate_point(rot,X)
  sqtheta = sum(rot.*rot)
  if sqtheta != 0.
    theta = sqrt(sqtheta)
    costheta = cos(theta)
...
compute_w_err_d = derivative(compute_w_err)

function get_compute_reproj_err_d(feat)
  compute_reproj_err_d = function(params)
    cam, X, w = unpack(params)
    return compute_reproj_err(cam,X,w,feat)
  end
  return compute_reproj_err_d
end

function compute_ba_J(cams,X,w,obs,feats)
  p = size(obs,2)
  reproj_err_d = Array(Float64,2*p,N_CAM_PARAMS + 3 + 1)
  for i in 1:p
    compute_reproj_err_d = get_compute_reproj_err_d(feats[:,i])
    idx = (2*(i-1))+1
    reproj_err_d[idx:idx+1,:] = jacobian(compute_reproj_err_d,
              pack(cams[:,obs[1,i]],X[:,obs[2,i]],w[i]))
  end
  w_err_d = Array(Float64,1,p)
  for i in 1:p
...
precompile(get_compute_reproj_err_d,(Vec,))
...
tJ = toq()/nruns_J; # should we add tf time?
@printf "tJ: %g\n" tJ
#println("J:")
#println(J)
